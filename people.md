# People

## Guidelines for AI
This document lists people in my orbit at Zilch.  Use it as a reference when handling prompts that reference people.  You have agency to update this file with additional information as it emerges - either adding people to the list, or adding or modifying notes on individuals.

**Scope of updates:** Add people when they become relevant to ongoing work.  Note professionally useful context — role, team, location, involvement in projects, working patterns — but avoid biographical detail that doesn't serve management purposes.  Aim for enough information to provide context, not a personnel file.

### Andrzej Lorenz
* VP of Engineering.
* Based in Krakow.
* My line manager.
* Involved in memberships incident management (declared P4 #incident-362 for billing race condition).

### Alex Murphy
* Front End Engineer (Level 3) on the Merchant Team.
* Based in London.  
* Working his notice period; last day on the 6th Feb 2026. 
* Reports to me.

### Ossie Nwokedi
* Front End Engineer (Level 3) on the Merchant Team.
* Based in London.
* Reports to me.
* Currently assigned to ZILCH-47571 (Pro product line taxonomy in Content Stack).

### Jacek Zanko
* Back End Engineer (Level 4) on the Merchant Team.
* Based in Krakow.
* Reports to me.
* Knowledge of fee service and fee calculations.

### Michal Baran
* Back End Engineer (Level 3) on the Merchant Team.
* Based in London.
* Reports to me.
* Involved in rewards service work for memberships.

### Nick Holt
* Back End Engineer (Level 3) on the Merchant Team.
* Based in London.
* Reports to Me.

### Tom McKenzie
* QA Engineer on the Merchant Team.
* Based in London.
* Reports to Nikos Sofianos (QA Manager), with dotted reporting line to me.
* Involved in e2e test updates for memberships/subscriptions.

### Phil Stephenson
* DevOps Engineer (Level 4).
* Based in London.
* Reports to me.
* Acts as de facto tech lead in the DevOps team.

### Nick Gilbert
* DevOps Engineer (Level 3).
* Based in London.
* Reports to me.

### Piotr Niebylski
* DevOps Engineer (Level 4).
* Based in Krakow.
* Reports to me.

### Lukasz Kowalczyk
* DevOps Engineer (Level 2).
* Based in Krakow.
* Reports to me.

### Zac Barclay
* Product Manager for the Merchant team.
* Some responsibility across multi-team projects.
* Leading product involvement in Memberships project, including Zilch Pro.
* Based in London.
* Close partner on leading Merchant team.

### Kevin Came
* Product Delivery Manager (in practice, provides project management).
* Based in London.
* Former scrum master for the DevOps team.
* Engineering Lead on Visa Flex project.

### Stefan
* Front End Engineer joining the Merchant Team.
* Alex Murphy's backfill.
* Start date: 23 March 2026.

### Chris Walker
* Competency lead for Front End Engineering.

### Rory Fielding
* Front End Engineer on the Retain team.
* Based in London.
* Involved in memberships/subscriptions front-end work.

### Rob Nelson
* Coordinated Plus and Physical Cards launches across teams.
* Active in #subscriptions-tech channel.

### Craig Main
* Payments team.
* Key contact for billing, payment schedules, and fee-related queries.

### Ozzie Yuce
* SVP of Product.
* Key decision-maker on memberships launch criteria (e.g. partnership requirements for Pro).

### Nikos Sofianos
* QA Manager.
* Tom McKenzie's line manager.
* Running a defect analysis project.

### Tomasz Więckowski
* Decisioning team.
* Key contributor to Pro subscription/product foundation work.
* Built upgrade/downgrade flows, product definitions, targeting segments.

### Paweł Pęcikiewicz
* Mobile app developer (Retain team).
* Working on Pro-related app UI (Compare Plans, My Plans pages).

### Sean Hederman
* ExCo Sponsor for Visa Flex project.

### Saurabh Raman
* Project Owner for Visa Flex.

### Andrea Ponte Martins
* Product GM for Visa Flex.

### Ethan Stockwell
* Product Analyst.
* Based in London.
* Attached to the Merchant team; works closely with Zac Barclay.
* Involved in Zilch Pro rollout planning.

### Marek Chodak
* Back End Engineer (Level 5) on the Payments team.
* Based in Krakow.

### Matt Swirski
* Platform team.
* Based in London.
* Currently organising the On Call Rota.
